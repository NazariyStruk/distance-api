...
public class AktService {
    ...
    private LocalDate normalizeDate(String rawDate) {
        ...
        // 1. Очищення рядка від " р.", " р", зайвих пробілів
        ...
        // Перевірка текстового формату (напр., "31 травня 2026")
        // Оскільки лапки видалено, cleanDate тепер дорівнює "31 травня 2026",
        // і метод split спрацює ідеально.
    ...
    private BigDecimal parseBigDecimal(String rawAmount) {
        ...
        // === ДОДАНО ДЛЯ ВИРІШЕННЯ ПРОБЛЕМИ З ВАЛЮТОЮ ("грн.") ===
        // 3. Видаляємо всі літери (кирилицю та латиницю), які могли залишитись
        // Наприклад: "252.00грн." перетвориться на "252.00."
        cleanAmount = cleanAmount.replaceAll("[а-яА-Яa-zA-ZіІїЇєЄґҐ]+", "");

        // 4. Якщо в самому кінці рядка залишилася крапка від скорочення "грн.", видаляємо її
        // Наприклад: "252.00." перетвориться на "252.00"
